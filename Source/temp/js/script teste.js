...
function carregarDados() {
    firebase.database().ref('estoqueBarrasTeste').once('value').then(function(snapshot) {
    ...
    firebase.database().ref('pecasRequeridasTeste').once('value').then(function(snapshot) {
...
function salvarDados() {
    firebase.database().ref('estoqueBarrasTeste').set(estoqueBarras);
    firebase.database().ref('pecasRequeridasTeste').set(pecasRequeridas);
}
...
function gerarRelatorioPDF() {
    ...
    doc.setFontSize(16);
    doc.text("OTIMIZAÇÃO DE BARRAS TESTE", 50, y);
    y += 10;
    doc.setFontSize(12);
