...
function carregarDados() {
    firebase.database().ref('estoqueBarrasc32').once('value').then(function(snapshot) {
    ...
    firebase.database().ref('pecasRequeridasc32').once('value').then(function(snapshot) {
...
function salvarDados() {
    firebase.database().ref('estoqueBarrasc32').set(estoqueBarras);
    firebase.database().ref('pecasRequeridasc32').set(pecasRequeridas);
}
...
function atualizarEstoque() {
    ...
    estoqueBarras.forEach((b, index) => {
        if (b.quantidade > 0) {
            const peso = (b.quantidade * b.comprimento * 0.0632).toFixed(2);
...
function gerarRelatorioPDF() {
    ...
    doc.setFontSize(16);
    doc.text("OTIMIZAÇÃO DE BARRAS Ø32 - CAVICON", 50, y);
    y += 10;
    doc.setFontSize(12);
...
function gerarListaEstoquePDF() {
    ...
    doc.setFontSize(16);
    doc.text("LISTA DE ESTOQUE CAVICON Ø32", 10, y);
    y += 10;
