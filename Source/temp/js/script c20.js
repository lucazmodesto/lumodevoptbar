...
function carregarDados() {
    firebase.database().ref('estoqueBarrasc20').once('value').then(function(snapshot) {
    ...
    firebase.database().ref('pecasRequeridasc20').once('value').then(function(snapshot) {
...
function salvarDados() {
    firebase.database().ref('estoqueBarrasc20').set(estoqueBarras);
    firebase.database().ref('pecasRequeridasc20').set(pecasRequeridas);
}
...
function atualizarEstoque() {
    ...
    estoqueBarras.forEach((b, index) => {
        if (b.quantidade > 0) {
            const peso = (b.quantidade * b.comprimento * 0.0246).toFixed(2);
...
function gerarRelatorioPDF() {
    ...
    doc.setFontSize(16);
    doc.text("OTIMIZAÇÃO DE BARRAS Ø20 - CAVICON", 50, y);
    y += 10;
    doc.setFontSize(12);
...
function gerarListaEstoquePDF() {
    ...
    function adicionarCabecalho() {
        doc.setFontSize(16);
        doc.text("LISTA DE ESTOQUE CAVICON Ø20", 10, y);
        y += 10;
        doc.setFontSize(12);
    ...
    // Abre o PDF no navegador
    const nomeArquivo = prompt("Digite o nome do arquivo (sem extensão):", "lista-estoque");
    doc.output("dataurlnewwindow");
}
